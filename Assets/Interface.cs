...
public class Interface : MonoBehaviour
{
    ...
    private void FixedUpdate() //Check for commands 60 times a second
    {
        ...
        {
            playerCube.transform.Translate(0.0f, 15f, 0.0f);
    ...
    void ParseTouch(string cmd)
    {
        try
        {
            string[] splitResult = cmd.Split(stringDelimiters, StringSplitOptions.RemoveEmptyEntries);
            byte x = byte.Parse(splitResult[0]);
            byte y = byte.Parse(splitResult[1]);
            byte z = byte.Parse(splitResult[2]);
            Vector3 touch = new Vector3(x, y, z);
            //Camera.main.GetComponent<Cube>().transform.Translate(0.0f, 15f, 0.0f);
            //Camera.main.GetComponent<Cube>().transform.Translate(touch.x, touch.y, touch.z);
            playerCube.transform.Translate(0.0f, 15f, 0.0f);
        }
